...
int main() {
        ...
        struct gameState G, testG, G1, testG1;
        ...
        r = initializeGame(numPlayers, k, seed, &G1);

        G1.whoseTurn = 1;
        G1.handCount[playerNum] = handCount;

        memcpy(G1.hand[playerNum], coppers, sizeof(int) * handCount);
        ...
        printf("New Hand Count = %d, Expected Count = %d\n", testG1.handCount[playerNum], G1.handCount[playerNum] + 2);
        success = assertTrue(testG1.handCount[playerNum], G1.handCount[playerNum] + 2, success);
